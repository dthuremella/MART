...
from loaders.dataloader_eth import TrajectoryDataset, use_kalman
...
def print_flops(model):
    ...
    print(f"\n=== FLOPs Report ===")
    print(f"[{args.dataset}] Total FLOPs: {total_flops:.3e}")
...
def main():
    ...
    if use_kalman:
        for name, module in model.named_modules():
            if isinstance(module, FMoETransformerMLPHarmonic):
                module.data = args.dataset
...
def train(epoch, model, optimizer, loader, train_EM_targets=False):
    ...
    for i, data in enumerate(loader):
        ...
        if batch_count % opts.batch_size == 0 or i == loader_len - 1: #uselastiter, remove from or if not using
            loss = loss / divider
            is_first_loss = True
...
def test(epoch, model, loader):
    ...
    if measure_flops: print('[{}] model FLOPs for one batch (size {}): {:.3e}'.format(args.dataset.upper(), batch_size, flops_per_batch))
